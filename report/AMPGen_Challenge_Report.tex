...
\addlinespace[1pt]
v2 & homology & \textbf{0.585} & \textbf{0.597} & \textbf{0.503} & \textbf{0.522} \\
...
\caption{\textbf{Deployed v2 predictor, category-level ranking on both evaluation splits.} Bars are
category-level Spearman $\rho$ (strain-weighted per-species MIC via the panel aggregation of
\S\ref{sec:agg}) plus the HC50 safety endpoint, for the shipped v2 model. Blue = \emph{homology}
split (CD-HIT-80\% whole-cluster; the honest, deployment-relevant number); purple = \emph{random}
by-sequence split (the inflated, literature-comparable number). Grey $\Delta$ = random$-$homology,
the honest penalty from removing near-duplicate leakage --- a modest, uniform $+0.05$ to $+0.08$
across categories, i.e.\ the model is not merely memorising homologs. Both numbers are reported so no
category score can be read without its honest reference.}
\end{figure}
...
\begin{table}[h]\centering\small
\caption{Per-species MIC Spearman $\rho$ and Pearson $r$ for the recommended v2 model on \emph{both}
...
($\rho$ $+0.047$); (ii) the controlled data ablation (Table~\ref{tab:ablation}) shows AMPBench-MT genuinely improves
